...
import edu.usfca.cs.chat.util.ConfigUtil;
...
public class ClientFS extends FuseStubFS {
    ...
    public int readdir(
        ...
        if (respWrapper.getReaddirResp() == null) {
    ...
    public int open(String path, FuseFileInfo fi) {
        ...
        if (respWrapper.getOpenResp() == null) {
            logger.warning("Received improper response type");
            return -1;
        }
    ...
    public int getattr(String path, FileStat stat) {
        ...
        if (respWrapper.getGetattrResp() == null) {
            logger.warning("Received improper response type");
            return -1;
        }
    ...
    public int read(String path, Pointer buf,
        ...
        if (respWrapper.getReadResp() == null) {
            logger.warning("Received improper response type");
            /* Note: for read(), 0 indicates failure */
            return 0;
        }
    ...
    throws IOException {
        if (args.length != 2) {
            System.err.println("Usage: ClientFS <controller_hostname> <mount-point>");
            System.exit(1);
        }
        ...
        ClientFS fs = new ClientFS(hostname, ConfigUtil.controllerPort);
        ...
        try { 
            ...
            FileUtil.makeDirectory(mountPoint);
            FileUtil.cleanDirectory(mountPoint, false);
